...
export default function CommitteeList() {
  const { data, loading, error, refetch } = useAdminFetch("/admin/committee");
  ...
  async function handleDelete() {
    try {
      await adminSend("DELETE", `/admin/committee/${toDelete._id}`);
  ...
  async function move(idx, dir) {
    ...
    try {
      await adminSend("POST", "/admin/committee/reorder", { ids });
      refetch();
    } catch (err) {
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: "role", label: "Role" },
            {
              key: "isExecutive",
              label: "Executive",
              render: (r) => (r.isExecutive ? <Badge>Executive</Badge> : "—"),
            },
